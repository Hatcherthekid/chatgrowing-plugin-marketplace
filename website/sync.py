...
def render(rows, template):
    ...
    for r in rows:
        ...
        cards.append(f'<article class="release" id="{esc(anchor)}" data-version="{esc(version)}"><aside><time datetime="{esc(r["date"])}">{esc(date)}</time><span class="status">{esc(label)}</span></aside><div class="article-body"><span id="{esc(old_anchor)}"></span><h2>{esc(r["title"])}</h2>'+markdown(r['body'])+f'<p><a href="{esc(r["url"], quote=True)}">{source_label} ↗</a></p></div></article>')
