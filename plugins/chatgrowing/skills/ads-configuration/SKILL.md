...
新任务先调用 `ads_capability_context`。只使用响应中当前 principal 可见的资源、能力、目标 facet、destination 与 server guidance。App facet 不授予权限，也不与账户强绑定；若服务端返回显式 App 级授权资源，按其 resource_ref 和可用能力使用，不能把账户授权扩展为整 App 授权。
